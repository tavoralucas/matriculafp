	...
	<%
		...
		if (CPF != "")
		{
			...
			if (dadosCO.HasRows)
			{
				...
				CUPOM = dadosCO["CUPOM"].ToString();
				DIA_PAG = dadosCO["DIA_PAG"].ToString();
		...
		if (Request.Form.Count > 0)
		{
			...
			if (!string.IsNullOrEmpty(Request.Form["cbDIA"]))
			{
				DIA_PAG = Request.Form["cbDIA"].ToString();
			}

			if (!string.IsNullOrEmpty(Request.Form["txtCUPOM"]))
			{
				CUPOM = Request.Form["txtCUPOM"].ToString();
			}
		...
		if ((comandogeral == "edit") && (comando=="SAVE"))
		{
			...
			MySqlConnection connDesconto = new MySqlConnection("server=matriculasfp.mysql.uhserver.com;User Id=matriculasfp;database=matriculasfp; password=c2007s**");
			connDesconto.Open();
			MySqlCommand sqldesconto = new MySqlCommand();
			sqldesconto.Connection = connDesconto;
			sqldesconto.CommandText = "Select * from CUPOM_DESCONTO where cupom='" + CUPOM+"'";
			MySqlDataReader dadosdesconto = sqldesconto.ExecuteReader();
			dadosdesconto.Read();

			decimal desconto = 0;

			if (dadosdesconto["VALOR"].ToString() != "")
			{
				desconto = Convert.ToDecimal(dadosdesconto["VALOR"]);
			}
			...
			if (desconto > 0)
			{
				for (int cont = 0; cont < payment.Items.Count; cont++)
				{
				  payment.Items[cont].Amount = payment.Items[cont].Amount - (desconto / payment.Items.Count);
				}
			}
	...
	%>
                                    ...
                                    <div class="form-body">
										...
										<div class="row">
									       ...
									       <div class="col-md-4">
                                                <div class="form-group">
                                                    <label for="txtCUPOM">Cupom DESCONTO</label>
                                                    <input type="text" class="form-control" id="txtCUPOM" name="txtCUPOM" value="<%=CUPOM%>" onblur="buscaAjax();"> </div>
                                            </div>

                                                <div class="col-md-4">
                                                <div class="form-group">
                                                    <label for="cbDIA">Dia do vencimento</label>
                                                    
													   <select class="custom-select form-control" id="cbDIA" name="cbDIA" required data-validation-required-message="preencher campo obrigatório">
                                                       <option value="<%=DIA_PAG %>"><%=DIA_PAG %></option>
  ...
  <script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script>
   ...
   <script src="assets/plugins/icheck/icheck.min.js"></script>
    ...
    $(function () {
		...
		<script type="text/javascript">
			$(function () {
				$("[id$=txtCUPOM]").blur(function () {

					if ($("[id$=txtCUPOM]").val() != "") {

						$.ajax({
							url: '<%=ResolveUrl("cadastroresp.aspx/GetCupom") %>',
							data: "{ 'prefixo': '" + $("[id$=txtCUPOM]").val() + "'}",
							dataType: "json",
							type: "POST",
							contentType: "application/json; charset=utf-8",
							success: function (data) {
...
<script>
	var x = document.getElementById("txtCUPOM");
	x.addEventListener("focus", myFocusFunction, true);
	//x.addEventListener("blur", myBlurFunction, true);

	function myFocusFunction() {
		document.getElementById("txtCUPOM").style.backgroundColor = "yellow";
	}
